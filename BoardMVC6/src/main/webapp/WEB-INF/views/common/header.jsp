      ...
      <c:if test="${!empty mvo}">
	      <ul class="nav navbar-nav navbar-right">
	            ...
	            <c:if test="${!empty mvo}">
			      ...
			      <li><img class="img-circle" src="${cp}/resources/images/person.PNG" style="width: 50px; height: 50px"/> ${mvo.memName} 님
			      (
			    ...
			    <c:if test="${!empty mvo.memProfile}">
			      <li><img class="img-circle" src="${cp}/resources/upload/${mvo.memProfile}" style="width: 50px; height: 50px"/> ${mvo.memName} 님
			      (
			      <c:forEach var="authVO" items="${mvo.authList}">
			          <c:if test="${authVO.auth eq 'ROLE_USER'}">U</c:if>
			          <c:if test="${authVO.auth eq 'ROLE_MANAGER'}">M</c:if>
